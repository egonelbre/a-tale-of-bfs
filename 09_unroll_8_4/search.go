...
func BreadthFirst(g *graph.Graph, source graph.Node, level []int) {
	...
	for len(currentLevel) > 0 {
		for _, node := range currentLevel {
			...
			for ; i < len(neighbors)-7; i += 8 {
				n1, n2, n3, n4 := neighbors[i], neighbors[i+1], neighbors[i+2], neighbors[i+3]
				n5, n6, n7, n8 := neighbors[i+4], neighbors[i+5], neighbors[i+6], neighbors[i+7]
				...
				if !visited.Contains(n5) {
					visited.Add(n5)
					nextLevel = append(nextLevel, n5)
				}
				if !visited.Contains(n6) {
					visited.Add(n6)
					nextLevel = append(nextLevel, n6)
				}
				if !visited.Contains(n7) {
					visited.Add(n7)
					nextLevel = append(nextLevel, n7)
				}
				if !visited.Contains(n8) {
					visited.Add(n8)
					nextLevel = append(nextLevel, n8)
				}
			}
